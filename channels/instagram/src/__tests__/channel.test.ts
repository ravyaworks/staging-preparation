...
import { InstagramChannel } from '../channel'
...
describe('InstagramChannel', () => {
  ...
  describe('initialization', () => {
    it('should have correct type, displayName, and version', () => {
      expect(channel.type).toBe('instagram')
      expect(channel.displayName).toBe('Instagram Messenger')
    ...
    it('should initialize with valid config', async () => {
      ...
      expect(config.customConfig?.instagramBusinessAccountId).toBe('ig-business-123')
    })
  })
  ...
  describe('connection', () => {
    ...
    it('should throw on connect without instagramBusinessAccountId', async () => {
      await channel.initialize({ enabled: true, customConfig: { appSecret: 'secret' } })
      await expect(channel.connect(validAuth)).rejects.toThrow('instagramBusinessAccountId is required')
    })
  ...
  describe('sendMessage', () => {
    ...
    it('should throw when not connected', async () => {
      const disconnected = new InstagramChannel()
    ...
    it('should send message with quick replies', async () => {
      const msg = createTestMessage('text', { text: 'Choose one' }, [], [], [], [
        { id: 'qr-1', title: 'Yes' },
        { id: 'qr-2', title: 'No' },
      ])
      const result = await channel.sendMessage(msg)
    ...
    it('should send image message with attachment', async () => {
  ...
  describe('config validation', () => {
    ...
    it('should return errors for missing instagramBusinessAccountId', () => {
      const errors = channel.validateConfig({ enabled: true, customConfig: { appSecret: 'secret' } } as ChannelConfig)
      expect(errors).toContain('instagramBusinessAccountId is required')
    })
  })
  ...
  describe('processIncoming', () => {
    it('should process text message webhook payload', async () => {
      ...
      expect(messages[0]?.content.text).toBe('Hello from Instagram!')
      expect(messages[0]?.user.id).toBe('sender-123')
    })

    it('should process image message with attachment', async () => {
      await channel.initialize(validConfig)
      const payload = createWebhookPayload({
        sender: { id: 'sender-123' },
        recipient: { id: 'ig-business-123' },
      ...
      expect(messages[0]?.attachments[0]?.url).toBe('https://example.com/img.jpg')
    })
    ...
    it('should process story mention', async () => {
      await channel.initialize(validConfig)
      const payload = createWebhookPayload({
        sender: { id: 'sender-123' },
        recipient: { id: 'ig-business-123' },
      ...
      expect(messages).toHaveLength(1)
      expect(messages[0]?.content.text).toContain('Story mention')
    })

    it('should ignore echo messages', async () => {
      await channel.initialize(validConfig)
      const payload = createWebhookPayload({
        sender: { id: 'sender-123' },
        recipient: { id: 'ig-business-123' },
  ...
  describe('webhook verification', () => {
    ...
    it('should verify with correct token', () => {
      const result = channel.verifyWebhook('subscribe', 'verify-token-123', 'challenge-abc')
      expect(result.verified).toBe(true)
      expect(result.challenge).toBe('challenge-abc')
    })

    it('should not verify with incorrect token', () => {
      const result = channel.verifyWebhook('subscribe', 'wrong-token', 'challenge-abc')
...
function createTestMessage(
  ...
  return {
    ...
    conversation: { id: 'conv-1', channelType: 'instagram' },
    tenant: { id: 'tenant-1' },
  }
}
